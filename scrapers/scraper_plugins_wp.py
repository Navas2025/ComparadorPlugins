#!/usr/bin/env python3
"""
Scraper para plugins de plugins-wp.online
Extrae: nombre, versión, URL
Guarda en: data/plugins_wp.csv
"""
import requests
...
def scrape_plugins_wp(max_pages=5):
    """
    Scrapea plugins de https://plugins-wp.online/categoria-producto/plugins/
    """
    print("=" * 60)
    print("Scrapeando plugins de plugins-wp.online")
    print("=" * 60)
    ...
    for page_num in range(1, max_pages + 1):
        try:
            if page_num == 1:
                url = f"{base_url}/categoria-producto/plugins/"
            else:
                url = f"{base_url}/categoria-producto/plugins/page/{page_num}/"
...
if __name__ == '__main__':
    try:
        plugins = scrape_plugins_wp(max_pages=5)
        save_to_csv(plugins, 'plugins_wp.csv')
        print(f"\n{'=' * 60}")
